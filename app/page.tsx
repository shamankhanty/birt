...
function hearingMetricsForDisplay(
  ...
  const order = { worsened: 0, improved: 1, same: 2, na: 3 };
  return [...row.metrics]
    .filter(
      (metric) =>
        filter === "all" ||
    ...
    .sort(
      (a, b) =>
        order[hearingMetricChange(row.key, a).direction] -
          order[hearingMetricChange(row.key, b).direction] ||
