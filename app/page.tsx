...
const baselineIndicators: Indicator[] = [
  {
    ...
    lag: 637,
    quantity: 31969,
    quantityLabel: "из 32 606 поданных заявлений",
    compareTo: "07.09.2026",
  },
  {
    id: "egpu2days",
    group: "Цифровые сервисы",
  ...
  },
  {
    ...
    lag: 56862,
    quantity: 602974,
    quantityLabel: "из 659 836 карт вызова",
    compareTo: "28.08.2026",
  },
  {
    id: "errors",
    group: "Качество",
    name: "Количество ошибок регистрации СЭМД за последнюю полную неделю",
    fact: 2046140,
    plan: null,
    unit: "",
    trend: null,
...
const methodologies: Methodology[] = [
  ...
  },
  {
    ...
    source: "РЭМД ЕГИСЗ — числитель; ФОМС / отчёт по госпитализациям — знаменатель",
    cadence: "Еженедельно; накопительно с начала года",
    note: "Числитель сопоставляется со знаменателем строго по OID МО. После исправления источника числителя динамика к прежнему расчёту не показывается; августовский набор по МО исключён из рейтинга до пересчёта по полному РЭМД-источнику.",
  },
  {
    id: "ambulatoryCase",
    name: "Доля СЭМД «Эпикриз по законченному случаю амбулаторный» (СЭМД №92, №233) относительно количества случаев",
  ...
  },
  {
    id: "errors",
    name: "Количество ошибок регистрации СЭМД за последнюю полную неделю",
    formula: "Сумма количества отказов регистрации СЭМД за последний полный недельный срез",
    numerator:
    ...
    cadence: "Еженедельно; последний полный недельный срез",
    note: "Показано оперативное абсолютное количество отказов за последнюю полную неделю. Доля и оценка качества без знаменателя всех обработанных запросов не рассчитываются.",
  },
  {
    id: "shortInput",
...
function hearingDisplayGroup(metric: HearingMetric): HearingDisplayGroup {
  if (metric.id.startsWith("operational500_")) return hearingDisplayGroups.doctors;
  if (
...
const versionHistory = [
  {
    ...
    date: "14.09.2026",
    items: [
      "Оперативные данные обновлены: ЕПГУ и АСУ СМП — по 11.09; ЭПЛ и ошибки РЭМД — по полной неделе 07–13.09.2026.",
      "Ошибки РЭМД переведены с накопительного массива на оперативный недельный контроль с разрезом по категориям и медицинским организациям.",
  ...
  {
    version: "5.0–5.3",
    date: "Неделя 07–13.09.2026",
    items: [
      "Рейтинг переведён на полный август с динамикой к июлю; оперативные сентябрьские срезы отделены от месячного рейтинга.",
      "Создан отдельный раздел МАХ: официальные годовые факты РТ отделены от оперативной активности МО; записи к врачу показаны без искусственной привязки к МО.",
      "Добавлены актуальные врачебные показатели и 500+, уточнена применимость по типу МО и числу врачей.",
      "Ошибки РЭМД получили разрез по МО и взаимную расшифровку категория ↔ медицинская организация.",
      "Конвейер обновления переведён на PASS/WARNING/FAIL и компактный source workspace для следующих недельных загрузок.",
    ],
  },
  {
    version: "4.2–4.3",
    date: "Неделя 31.08–06.09.2026",
    items: [
      "Обновлены амбулаторные эпикризы, профилактика 122/228 и федеральный отчёт по активности врачей.",
      "Переходная формула профилактики 2026 года закреплена как MAX(СЭМД 122; СЭМД 228) отдельно по каждой МО.",
      "Для заслушанных МО расширена фиксация baseline и динамики; отсутствие данных не трактуется как невыполнение.",
      "Раздел методик дополнен формулой, числителем, знаменателем, источником и практическими действиями МО.",
    ],
  },
  {
    version: "3.5–4.1",
    date: "Неделя 24–30.08.2026",
    items: [
            ...
            <>
              ...
              <div className="hearingStats">
                <article>
                  ...
                  <span>суммарного индекса влияния на цели РТ</span>
                </article>
              ...
              <details className="hearingMethod hearingTopTen" open>
                ...
                <em>
                  ТОП-10: индекс {format(hearingImpactSummary.topTenContribution, 3)}, или {format(hearingImpactSummary.topTenShare, 1)}% суммарного индекса влияния.
                </em>
              </details>
                    ...
                    >
                      ...
                      {row.heard && snapshotSummary.snapshot && (
                        <div className="hearingSnapshotSummary">
                          ...
                          <p>
                            <small>Текущий индекс влияния</small>
                            <b>{format(row.regionalContribution, 3)}</b>
                      ...
                      <div className="hearingSignals">
                        ...
                        <p>
                          <small>Показателей, влияющих на РТ</small>
                          <b>{row.affecting.filter((metric) => (metric.regionalContribution ?? 0) > 0).length}</b>
                        </p>
                        <p>
                                    ...
                                    Статус: {metric.applicable === false
                                      ...
                                      : metric.status === "missing"
                                        ...
                                        : metric.regionalContribution !== null && metric.regionalContribution !== undefined
                                          ...
                                          : "Справочно"}
                                    {metric.regionalContribution !== null && metric.regionalContribution !== undefined
                                      ? ` · вклад в недостижение РТ ${format(metric.regionalContribution, 3)} п.п.`
                                      : ""}
                                  </small>
              ...
              <section className="waybillDynamics">
                ...
                {waybillMode === "week" ? (
                  <div className="dynamicsCards waybillCards eplWeeklyCards">
                    ...
                    <article className={waybillMovementDelta >= 0 ? "positive" : "negative"}>
                      <small>Изменение движения</small>
                      <strong>
                        {waybillMovementDelta > 0 ? "↑" : waybillMovementDelta < 0 ? "↓" : "→"}{" "}
                        {format(Math.abs(waybillMovementDelta), 2)} п.п.
                      </strong>
                      <span>
                        +
                        {format(
                    ...
                    <article className={waybillCoverageDelta >= 0 ? "positive" : "negative"}>
                      <small>ТС с путевыми листами</small>
                      <strong>
                        {waybillCoverageDelta > 0 ? "↑" : waybillCoverageDelta < 0 ? "↓" : "→"}{" "}
                        {format(Math.abs(waybillCoverageDelta), 2)} п.п.
                      </strong>
                      <span>
                        {format(
                          electronicWaybillWeekly.previous.systemSummary
