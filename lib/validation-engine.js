  ...
  for (const [scope, datasets] of sources) {
    for (const [metric, dataset] of Object.entries(datasets ?? {})) {
      ...
      for (const row of dataset.rows) {
        ...
        else if (row?.sourceStatus === "external_source") {
          // Kept in the source data explicitly, but not an MO eligible for
          // the approved registry/rating. This is a visible source condition,
          // not a synthetic zero or an identity match.
          continue;
        }
        else {
          unresolved.push({
        ...
        if (row?.sourceStatus !== "no_source_row" && typeof previous === "number" && (current === null || current === undefined)) {
          missingCurrent.push({
