...
export function resolveReportingPeriods({ monthlyDatasets = {}, datasets = [], physicianMetrics = null, fallbackYear = 2026 } = {}) {
  ...
  if (physicianMetrics) {
    ...
    // A physician report may be an operational partial-month snapshot.  It
    // becomes a reporting-month candidate only when the adapter explicitly
    // marks it as a complete calendar month.
    const physicianCandidate = candidateFromDataset({
      ...
      periodType: physicianMetrics.periodType,
      periodKind: physicianMetrics.periodKind,
    });
